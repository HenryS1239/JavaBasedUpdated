...
public class PNControllerTemp {
	...
	/*
	 * @RequestMapping("/notify.html") public String doLogin(Model
	 * model, @ModelAttribute("userid") String userid,
	 * @ModelAttribute("groupid") String groupid, @ModelAttribute("password") String
	 * password, HttpServletRequest req) {
	 * 
	 * // Get spring bean from the autowired web application context. //
	 * UserAccountBean accountBean = //
	 * (UserAccountBean)webContext.getBean("userAccountBean"); SqlSearchBean
	 * searchDBean = (SqlSearchBean) webContext.getBean("searchDBean");
	 * 
	 * // boolean checkResult = accountBean.checkUserLogin(userid, groupid,
	 * password); boolean result = searchDBean.searchDB(userid, groupid, password);
	 * 
	 * if (result) { // result) return "login_success"; } else {
